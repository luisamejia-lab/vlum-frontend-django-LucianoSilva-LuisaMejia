  ...
  const handleDelete = async () => {
    if (window.confirm(`¿Estás seguro de que deseas eliminar la película "${movie.title}"?`)) {
      try {
        ...
        window.location.reload(); // Refresca la lista inmediatamente
      } catch (error) {
  ...
  return (
    <Card
      sx={{
        ...
        '&:hover': { transform: 'translateY(-8px) scale(1.02)', borderColor: '#C2185B', boxShadow: '0 20px 30px -10px rgba(194, 24, 91, 0.4)' }
      }}
    >
      <CardMedia component="img" image={`${mediaUrl}/${movie.picture}`} alt={movie.title} sx={{ height: 320, objectFit: 'cover' }} />
      <CardContent sx={{ flexGrow: 1, p: 2 }}>
        <Typography gutterBottom variant="h6" component="div" sx={{ fontWeight: 700, color: '#fff' }}>{movie.title}</Typography>
        <Typography variant="body2" sx={{ color: '#4DB6AC', fontWeight: 600 }}>{movie.year || "Categoría"}</Typography>
      </CardContent>
      <CardActions sx={{ p: 2, pt: 0, display: 'flex', alignItems: 'center' }}>
        <Button size="small" component={Link} to={`/movies/${movie.id}`} sx={{ color: '#FFAB91', fontWeight: 600 }}>Ver Más</Button>
